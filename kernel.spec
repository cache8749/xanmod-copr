...
%global package_name kernel-xanmod-edge
%global source_name kernel
%global gemini 0
...
%define all_configs %{source_name}-%{specrpmversion}-*.config
...
Source0: https://gitlab.com/xanmod/linux/-/archive/%{tarfile_release}-xanmod1/linux-%{tarfile_release}-xanmod1.tar.bz2

Source1: Makefile.rhelver
Source2: %{source_name}.changelog
...
%if 0%{?include_rhel}
Source24: %{source_name}-aarch64-rhel.config
Source25: %{source_name}-aarch64-debug-rhel.config
Source27: %{source_name}-ppc64le-rhel.config
Source28: %{source_name}-ppc64le-debug-rhel.config
Source29: %{source_name}-s390x-rhel.config
Source30: %{source_name}-s390x-debug-rhel.config
Source31: %{source_name}-s390x-zfcpdump-rhel.config
Source32: %{source_name}-x86_64-rhel.config
Source33: %{source_name}-x86_64-debug-rhel.config
# ARM64 64K page-size kernel config
Source42: %{source_name}-aarch64-64k-rhel.config
Source43: %{source_name}-aarch64-64k-debug-rhel.config

Source44: %{source_name}-riscv64-rhel.config
Source45: %{source_name}-riscv64-debug-rhel.config
%endif
...
#Source52: %{source_name}-aarch64-fedora.config
#Source53: %{source_name}-aarch64-debug-fedora.config
#Source54: %{source_name}-aarch64-16k-fedora.config
#Source55: %{source_name}-aarch64-16k-debug-fedora.config
#Source56: %{source_name}-ppc64le-fedora.config
#Source57: %{source_name}-ppc64le-debug-fedora.config
#Source58: %{source_name}-s390x-fedora.config
#Source59: %{source_name}-s390x-debug-fedora.config
Source60: %{source_name}-x86_64-fedora.config
Source61: %{source_name}-x86_64-debug-fedora.config
#Source700: %{source_name}-riscv64-fedora.config
#Source701: %{source_name}-riscv64-debug-fedora.config
...
%if 0%{include_rt}
%if 0%{include_rhel}
Source474: %{source_name}-aarch64-rt-rhel.config
Source475: %{source_name}-aarch64-rt-debug-rhel.config
Source476: %{source_name}-aarch64-rt-64k-rhel.config
Source477: %{source_name}-aarch64-rt-64k-debug-rhel.config
Source478: %{source_name}-x86_64-rt-rhel.config
Source479: %{source_name}-x86_64-rt-debug-rhel.config
%endif
%if 0%{include_fedora}
#Source480: %{source_name}-aarch64-rt-fedora.config
#Source481: %{source_name}-aarch64-rt-debug-fedora.config
#Source482: %{source_name}-aarch64-rt-64k-fedora.config
#Source483: %{source_name}-aarch64-rt-64k-debug-fedora.config
Source484: %{source_name}-x86_64-rt-fedora.config
Source485: %{source_name}-x86_64-rt-debug-fedora.config
#Source486: %{source_name}-riscv64-rt-fedora.config
#Source487: %{source_name}-riscv64-rt-debug-fedora.config
%endif
%endif

%if %{include_automotive}
%if %{with_automotive_build}
Source488: %{source_name}-aarch64-rhel.config
Source489: %{source_name}-aarch64-debug-rhel.config
Source490: %{source_name}-x86_64-rhel.config
Source491: %{source_name}-x86_64-debug-rhel.config
%else
Source488: %{source_name}-aarch64-automotive-rhel.config
Source489: %{source_name}-aarch64-automotive-debug-rhel.config
Source490: %{source_name}-x86_64-automotive-rhel.config
Source491: %{source_name}-x86_64-automotive-debug-rhel.config
%endif
%endif
...
mv linux-%{tarfile_release}-xanmod1 linux-%{KVERREL}
...
cp $RPM_SOURCE_DIR/%{source_name}-*.config .
cp %{SOURCE80} .
# merge.py
cp %{SOURCE3000} .
...
FLAVOR=%{primary_target} SPECPACKAGE_NAME=%{source_name} SPECVERSION=%{specversion} SPECRPMVERSION=%{specrpmversion} ./generate_all_configs.sh %{debugbuildsenabled}
...
RHJOBS=$RPM_BUILD_NCPUS SPECPACKAGE_NAME=%{source_name} ./process_configs.sh $OPTS %{specrpmversion}
...
InitBuildVars() {
    ...
    Config=%{source_name}-%{specrpmversion}-%{_target_cpu}${Variant:+-${Variant}}.config
